            ...
            <div class="row">
                <div class="col-lg-8 mx-auto">
                    <form method="POST">
                        ...
                        <div class="control-group">
                            <div class="form-group controls mb-3 pb-2">
                                <input class="form-control" name="email" type="text" placeholder="Mail Adresi" required="required" />
                            </div>
                        </div>
                        <div class="control-group">
                            <div class="form-group controls mb-3 pb-2">
                                <input class="form-control" name="rezid" type="number" placeholder="Rezervasyon Numarası" required="required" />
                            </div>
                        </div>
                        <div class="form-group"><button class="btn btn-primary" type="submit">Rezervasyonu Bul</button></div>
                    </form>
                    <?php
                    if (isset($_POST['gsm']) && isset($_POST['email'])) {
                        $kontrol = mysqli_query($conn, "SELECT `rezid`,`email` FROM reservation WHERE rezid = '".$_POST["rezid"]."' AND email = '" .$_POST["email"]. "'");
                        if(mysqli_num_rows($kontrol))
                        {
                            header("location:rezervasyondegistir.php?rez=".$_POST["rezid"]."");
                            exit();
                        }
                        else {
